        ...
        for box in bbox:
            ...
            if cls_names == 1:
                ...
                if len(chars) == 8:
                    ...
                    raw_path = f"localhost:5000/{raw_images_dir}/raw_{timestamp}.jpg"
                    plt_path = f"localhost:5000/{plate_images_dir}/plt_{timestamp}.jpg"
                    ...
                    cursor.execute(
                        ...
                        (timestamp, raw_path, plt_path, persian_output, cameraId)
                    )
                    ...
                    #Send data to the server via socket (optional)
                    try:
                        data = {
                            "date": timestamp,
                            "raw_image_path": raw_path,
                            "plate_cropped_image_path": plt_path,
                            "predicted_string": persian_output,
                            "cameraid": cameraId
                        }
                        emit('plate_detected', data)
...
def get_license_data():
    try:
        ...
        #     raw_image_data = raw_image_file.read()

        # with open(plate_image_path, "rb") as plate_image_file:
        #     plate_image_data = plate_image_file.read()

        # raw_image_base64 = base64.b64encode(raw_image_data).decode("utf-8")
        # plate_image_base64 = base64.b64encode(plate_image_data).decode("utf-8")
...
def get_all_plates():
    try:
        ...
        if page == 0:
            # Fetch all records
            cursor.execute("SELECT id, date, predicted_string, raw_image_path, plate_cropped_image_path FROM plates")
...
raw_images_dir = os.path.join(os.getcwd(), 'images', 'raw')
plate_images_dir = os.path.join(os.getcwd(), 'images', 'plate')
...
def serve_raw_image(filename):
    ...
    raw_image_path = os.path.join(raw_images_dir, filename.replace("\\", "/"))
    if os.path.exists(raw_image_path):
        return send_file(raw_image_path, mimetype='image/jpeg')
    return jsonify({"error": "File not found"}), 404


@app.route('/images/plate/<path:filename>')
@cross_origin(supports_credentials=True)
def serve_plate_image(filename):
    """Serve plate images."""
    plate_image_path = os.path.join(plate_images_dir, filename.replace("\\", "/"))
    if os.path.exists(plate_image_path):
        return send_file(plate_image_path, mimetype='image/jpeg')
    return jsonify({"error": "File not found"}), 404
